...
pipeline {
  ...
  options { timestamps() } // ansiColor removed (plugin not installed)

  environment {
    REPO_SLUG = 'ShubhamUpadhay/SainjPortfolio' // <- matches your logs
    BRANCH    = 'main'
    GH_NAME   = 'jenkins-bot'
    ...
    stage('Build') {
      steps {
        sh '''
          export VITE_APP_VERSION=$GIT_COMMIT
          npm run build
          # Your vite.config.ts uses outDir: "build"
      ...
      steps {
        withCredentials([string(credentialsId: 'gh_pat', variable: 'GH_PAT')]) {
          sh '''
            ...
            # Sync build output (note: build/, not dist/)
            rsync -av --delete ./build/ "$WORKDIR"/
